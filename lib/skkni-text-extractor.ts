 ...
 * menjadi struktur Unit Kompetensi, Elemen, dan KUK.
 * Berjalan aman di Client (Browser) maupun Server (Node.js).
 */
export function parseSkkniText(rawText: string, totalHalaman?: number): ParsedSkkniDocument {
  ...
  const nomorMatch =
    rawText.match(/NOMOR\s+([0-9]+)\s+TAHUN\s+([0-9]{4})/i) ||
  ...
  const judulMatch =
    rawText.match(/TENTANG\s+PENETAPAN\s+STANDAR\s+KOMPETENSI\s+KERJA\s+NASIONAL\s+INDONESIA\s+([^.]+?)(?=\s+BAB|\s+DENGAN|\s+MEMUTUSKAN)/i) ||
  ...
  for (let idx = 0; idx < unitBlocks.length; idx++) {
    ...
    // Ekstraksi Judul Unit
    const judulMatch = block.match(
